...
def make_excel(logs, start_date, end_date):
    ...
    for log in logs:
        ...
        label = f"{d.month:02d}월 {d.day:02d}일({WEEKDAYS[d.weekday()]})"
        ...
        for col, val in enumerate(
            [label, log["department"], log["name"],
             int(odo_s), int(odo_e), int(dist),
             "X", int(dist), log["destination"] or "",
        ...
        if st.button("등록 / 정보 수정", key="btn_reg", use_container_width=True):
            if r_name and r_phone and r_emp:
                ...
                st.session_state.update(
                    ...
                    user_phone=r_phone,
                    user_name=r_name,
                    user_department=r_dept,
                    user_employee_id=r_emp,
                )
                st.success("등록 완료!")
                st.rerun()
            else:
...
def tab_reservation():
    ...
    with col_right:
        ...
        if not st.session_state.logged_in:
            ...
            dept_idx = (DEPARTMENTS.index(st.session_state.user_department)
...
def tab_pre_drive():
    ...
    with st.form("form_pre", clear_on_submit=True):
        c1, c2 = st.columns(2)
        with c1:
            p_dept = st.selectbox("부서", DEPARTMENTS, index=dept_idx)
    ...
    if pre_drives:
        ...
        for p in pre_drives:
            st.info(
                f"📅 **{p['drive_date']}**  │  "
                f"출발 계기판: **{p['odometer_start']:,.0f} km**  │  "
                f"목적지: {p['destination']}  │  "
                f"동행: {p['companions'] or '없음'}"
            )
